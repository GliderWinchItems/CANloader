...
printf("GAP: %7d 0x%08X  0x%08X %d\n\r",linect,(s3bin_prev.addr+s3bin_prev.ct),s3bin.addr,s3bin.addr-(s3bin_prev.addr+s3bin_prev.ct) );						
 ...
 //printf("%4d %s",bct, buf);
	...
	fwrite((uint8_t*)&ourbinsize,1,4,fpXbin);
	if ((ourbinsize & 0x3) != 0)
	{
	...
	/* Read in .bin file and compare to our local bin file.
	   ...
	   our local bin file will have 0xff. */
	int binfilectr = 0;
	uint8_t bb;
	rewind(fpBin);
...
exit(-5);	
	...
	int32_t ct = s3bin.addr-(s3bin_prev.addr+s3bin_prev.ct);
	if (ct < 1)
	{
		printf("NEG GAP: %d\n",ct);
		exit(-9);
	}
printf("BEGIN GAP GEN: %0x08x  %0x08x %d\n\r",s3bin.addr,s3bin_prev.addr,ct);
	...
	while (ct > 8)
	{
		makeline(8);
		ct -= 8;
		printf("LOOP  GAP GEN: %0x08x  %0x08x %d\n\r",s3bin.addr,s3bin_prev.addr,ct);

	} 
	if (ct > 0) // Complete remainder, if any
	{
		makeline(ct);
		printf("REMAINDER GEN: %0x08x  %0x08x %d\n\r",s3bin.addr,s3bin_prev.addr,ct);
